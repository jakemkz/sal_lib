...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	%%%
        ...
        function eff = bfce(dpoint)
            ...
            W   = tau_b*4*pi;
            mf  = sal_fuelinj(fuel_p,fuel_pw,'HF0437');
            LHV = 44*10^6;
        ...
        function [ca10, ca50, ca90] = hrstat(dpoint)
            ...
            ca90 = [];
            for i = 1:length(ca(1,:))
                [mfb(:,i), cad10, cad50, cad90] = sal_mfb(cyl1p(:,i),cyl_v,40,20,spk);
                %plot(mfb),hold on
                ca10 = [ca10 cad10];
                ca50 = [ca50 cad50];
                ca90 = [ca90 cad90];
        ...
        function [ind] = meanrealcycle(dpoint)
            cyl1p  = dpoint.getdata('4/CAD','cyl1p');
            r1map  = dpoint.getdata('4/CAD','map');
        ...
        %%% compute mean knocking cycle
        function [cyl_p, theta_knock] = meanknock(dpoint)
            cyl3p = dpoint.getdata('100000Hz','cyl3p');
            bdc   = dpoint.getdata('100000Hz','bdc');
            map   = dpoint.getdata('100000Hz','map');
            ca    = dpoint.getdata('4/CAD','theta');
            
            cyl3p = sal_cyclify(cyl3p,bdc,'peg');
            map   = sal_cyclify(map,bdc,'free');
